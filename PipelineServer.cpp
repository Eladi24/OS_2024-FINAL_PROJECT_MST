...
// sendResponse moved to ServerConnection namespace
  ...
  while (!terminateFlag.load()) {
    ...
    if (cmd == "Newgraph") {
      ...
      if (!graphGuard.owns_lock()) {
        ServerConnection::sendResponse(clientSock,
                                       "Graph is being used by another thread. "
                                       "Cannot initialize new graph.\n",
                                       coutLock);
        continue;
      }
      if (g != nullptr) {
      ...
      pipeline[0]->enqueue(
          [&ss, &n, &m, &g, &ssLock, &initLock, &initDone, &initCV, &res]() {
      ...
      // Validate graph parameters are logically possible
      // For an undirected graph with n vertices, maximum edges = n*(n-1)/2
      int maxEdges = (n * (n - 1)) / 2;
      if (m > maxEdges) {
        ServerConnection::sendResponse(
            clientSock,
            "Invalid graph parameters: A graph with " + to_string(n) +
                " vertices can have at most " + to_string(maxEdges) +
                " edges, but " + to_string(m) + " edges were requested.\n",
            coutLock);
        graphGuard.unlock(); // Release lock before continuing
        continue;
      }
...

        {
          ...
          if (!(ss >> u >> v >> w)) {
            ...
            if (!(ss >> u >> v >> w)) {
              ServerConnection::sendResponse(
                  clientSock,
                  "Invalid input format. Please enter 3 "
      ...
      pipeline[0]->enqueue([&g, u, v, w, &future, &done, &cv]() {
        ...
        unique_lock<mutex> futureGuard(futureLock);
        if (!success) {
          future = "Invalid edge. Vertices should be in the range [1, n] and "
                   "weight should be non-negative, or edge already exists.\n";
        } else {
          future = "Edge added between vertices " + to_string(u) + " and " +
    ...
    // Adding REMOVE_EDGE command handling
      ...
      pipeline[0]->enqueue([&g, u, v, &future, &done, &cv]() {
        ...
        unique_lock<mutex> futureGuard(futureLock);
        if (!success) {
          future = "Edge between vertices " + to_string(u) + " and " +
                   to_string(v) + " does not exist.\n";
        } else {
          future = "Edge removed between vertices " + to_string(u) + " and " +
                   to_string(v) + ".\n";
        }
        done.store(true, memory_order_release);
        cv.notify_one();
      ...
      pipeline[1]->enqueue([&g, cmd, &factory, &mst, &future, &done, &cv,
        ...
        if (!graphGuard.owns_lock()) {
          unique_lock<mutex> futureGuard(futureLock);
          future = "Graph is being used by another thread. Cannot search for "
                   "MST using " +
                   cmd + ".\n";
          done.store(true, memory_order_release);
          cv.notify_one();
...
// acceptConnection moved to ServerConnection namespace
