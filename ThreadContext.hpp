...
/**
 * @class ThreadContext
 * @brief Manages the lifecycle and synchronization of a single thread within a
 * thread pool.
 *
 * The ThreadContext class is responsible for creating, managing, and
 * synchronizing a thread. It provides methods to create a thread with a
 * specific function, wake up the thread, put the thread to sleep, and wait for
 * a condition to be met. This class is typically used within a thread pool to
 * manage individual threads. unlike std::thread, this class provides a way to
 * cancel a thread even if it is in a system blocking call. Those goals are
 * achieved by using pthreads.
 */
class ThreadContext {
private:
  ...
  unique_ptr<function<void()>>
      _functUniquePtr; // Unique pointer to the function to execute when
                       // creating the thread
...
public:
  ...
  /**
   * @brief Adds a file descriptor and its corresponding event handler to the
   * thread context.
   * @param fd The file descriptor to add.
   * @param event The event handler function to associate with the file
   * descriptor.
   */
   ...
   * This method is typically called by the thread pool to execute the event
  ...
  bool operator==(const ThreadContext &other) {
   ...
   * Changes the state of the thread to awake and notifies the condition
   * variable.
   */
  void wakeUp();
  ...
  bool isAwake() {
    return _isAwake.load(memory_order_acquire);
  } // Returns true if the thread is awake

  int getClientFd() const {
    return _clientFd;
  } // Returns the client file descriptor associated with the thread

  /**
   * @brief Waits for a condition to be met.
   * @param stopFlag The flag to check for the condition.
   * This method waits for the condition variable to be notified or the stop
   * flag to be set.
   */
