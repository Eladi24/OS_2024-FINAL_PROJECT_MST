...
std::string Tree::printMST(int node, int parent, std::vector<bool> &visited) {
    std::string result;
    visited[node] = true;

    for (auto& edge : adj[node]) {
        if (!visited[edge.dest - 1]) {
            result += std::to_string(node + 1) + " -- " + std::to_string(edge.dest) + " (" + std::to_string(edge.weight) + ")\n";
